...
from models.ahp_adapter.mmcv_custom import my_load_checkpoint as load_checkpoint
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., with_cp=False,
                 ...
                 windowed=False, window_size=14, use_residual=False, layer_scale=False, use_grad_checkpointing=False):
        super().__init__()
        ...
        # self.adaptercontroller1 = AdapterController(self.adapterconfig, 196, drop)
        ...
        # self.adaptercontroller2 = AdapterController(self.adapterconfig, 196, drop)
    ...
    def forward(self, x, H, W, register_hook):

        def _inner_forward(x, register_hook=register_hook):
            if self.layer_scale:
                x = x + self.drop_path(self.gamma1 * self.attn(self.norm1(x), H, W, register_hook=register_hook))
                # x = self.adaptercontroller1(x.permute(0, 2, 1)).transpose(1, 2).contiguous()
                x = x + self.drop_path(self.gamma2 * self.mlp(self.norm2(x)))
                # x = self.adaptercontroller2(x.permute(0, 2, 1)).transpose(1, 2).contiguous()
            else:
                x = x + self.drop_path(self.attn(self.norm1(x), H, W, register_hook=register_hook))
                # x = self.adaptercontroller1(x.permute(0, 2, 1)).transpose(1, 2).contiguous()
        ...
        if self.with_cp and x.requires_grad:
            ...
            x = _inner_forward(x, register_hook)
...
class TIMMVisionTransformer(BaseModule):
    ...
    def forward_features(self, x, register_blk=11):
