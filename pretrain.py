...
from models.ahp_pretrain import ahp_pretrain
import utils
...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    for i, (image, caption) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # loss = loss_ita + loss_lm
        if torch.distributed.get_rank() == 0:
            # wandb.log({'Bleu_val_1': coco_val.eval['Bleu_1'],
            #            'Bleu_val_2': coco_val.eval['Bleu_2'],
            #            'Bleu_val_3': coco_val.eval['Bleu_3'],
            #            'Bleu_val_4': coco_val.eval['Bleu_4'],
            #            'epoch': epoch,
            #            })
...
def main(args, config):
    ...
    model = ahp_pretrain(image_size=config['image_size'], vit=config['vit'], vit_grad_ckpt=config['vit_grad_ckpt'],
    ...
    print("模型可训练参数量: ", num_trainable_params)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='./AHP/output/Pretrain')
